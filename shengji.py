...
def dugeon_jiuguan_in():
    ...
    while True:
        ...
        screenshot_np = np.array(screenshot)
        screenshot_np = cv2.cvtColor(screenshot_np, cv2.COLOR_BGR2RGB)             
        loc,conf,w,h =find_image_on_screen(jiuguanditu_img,screenshot_np)
        print(conf)
        if conf>0.65:
            time.sleep(1) 
...
#酒馆再次挑战    
def jiuguanre_in(img_path,cha_type):
    global stuck
    zhuangebixiuli() #修装备
    zero_pilao_path ='.\\chc\\jiuguan0pilao.png' 
    kong_screen = screen[190:280,226:390]
    loc,conf,w,h=find_image_on_screen(zero_pilao_path,kong_screen) #0疲劳回城
    if conf>0.65: 
       return 1 
            ...
            while True:
             for img in shuatu_files:
                 ...
                 if shenyuan_flg ==1:
                    ...
                    continue            
                 if img_name == 're':           
                    fanhui_flg = jiuguanre_in(img_path,cha_type) #0为有疲劳 1为空疲劳返回
                    if cha_type==1: #选择攻击模式
                       sattack()
                    elif cha_type==0:
                       nattack()                      
